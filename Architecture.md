1. The Core Calculus Specification (λ 
total
LCP
​
 )
...
This defines how "Power" is checked. A program is sound if its internal effects ϵ are a subset of its granted capabilities K.
...
3. The Phase Separation & Erasure Protocol
...
reify<T>(): Converts a Stage-0 Type into a Stage-2 TypeToken.

split(linear Δ): The core logic for context splitting (Δ=Δ 
1
​
 ⊎Δ 
2
...
borrow(linear T): Temporarily treats a linear resource as unrestricted within a scope (Region-based).

quote/splice: The entry points for the Metaprogramming engine.
